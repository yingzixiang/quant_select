"""
统一数据获取层（共享底座 base/data_fetch.py）

日线行情、资金流向、指数数据、A股列表、流通市值。
收敛重构 P1：原 short_term/data_fetcher.py 的实现上移至此，short_term 仅保留兼容 shim。

数据源回退链（日线）：东方财富 stock_zh_a_hist → 腾讯 stock_zh_a_hist_tx → 新浪 stock_zh_a_daily
缓存：复用 core/data_cache.cached_api_call
"""
import sys
...
# 缓存 TTL（秒），与数据源更新频率对齐
PRICE_CACHE_TTL = 21600   # 价格数据 6 小时
FLOW_CACHE_TTL = 21600    # 资金流向 6 小时
INDEX_CACHE_TTL = 86400   # 指数数据 24 小时
...
def fetch_daily_kline_batch(codes: list, start_date: str, end_date: str,
    ...
    """
    批量获取日线OHLCV数据（东方财富数据源，并发拉取，含回退）
